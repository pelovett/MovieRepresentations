...
def split_transform_file(path_to_file: str, prepare_params: dict):
    ...
    source_df = pd.read_csv(path_to_file, delimiter='\t', names=[
    ...
    source_df = source_df.groupby('cust_id')['movie_id'].apply(
        list).reset_index(name='movies')

    print(f'Number of filtered customer records: {len(source_df)}')
    print(
        f"Mean number of movies rated: {source_df['movies'].apply(len).mean()}")
    print(
        f"Median number of movies rated: {source_df['movies'].apply(len).median()}")
    print(f'Number of movies: {num_movies}')

    # Split data into groups for model development
    train_data, val_data = train_test_split(source_df, test_size=1.0-train_size,
                                            train_size=train_size,
                                            random_state=random_seed)
    val_split_size = val_size / (1.0-train_size)
    val_data, test_data = train_test_split(val_data,
                                           test_size=1.0-val_split_size,
